...
const prompt = "tempmail"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "domain email inbox message webhook"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TEMPMAIL_APIKEY (api key), TEMPMAIL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TEMPMAIL_APIKEY carries the API key and
	// TEMPMAIL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TEMPMAIL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TEMPMAIL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
