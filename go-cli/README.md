...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tempmail-cli)
make build
...
./tempmail-cli list domain
./tempmail-cli list email

# 5. Override the API base URL for a single call
TEMPMAIL_BASE=https://api.example.com ./tempmail-cli list domain

# 6. No arguments -> interactive REPL
./tempmail-cli
tempmail> list domain
tempmail> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tempmail-cli
   ```
...
Run with no arguments to open a REPL (prompt `tempmail>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/tempmail-cli`. |
...
(`./tempmail-cli <expr>`) and interactively (the REPL), and expressions compose the
