...
two agent tools — `tempmail_list` and `tempmail_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tempmail-mcp)
make build

# 2. Provide credentials via the environment
export TEMPMAIL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user tempmail \
  -- /absolute/path/to/tempmail-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./tempmail-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// tempmail_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// tempmail_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tempmail-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user tempmail \
     -- "$PWD"/dist/*/tempmail-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `tempmail_list` and `tempmail_load` tools now appear
   in new sessions. Ask the agent to *"list domain using tempmail"*
   and it calls `tempmail_list` with `{"entity":"domain"}`.
...
```sh
export TEMPMAIL_APIKEY=sk_live_xxx            # API key
export TEMPMAIL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `tempmail_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `tempmail_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TEMPMAIL_APIKEY` | API key sent with every request. |
| `TEMPMAIL_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

domain | email | inbox | message | webhook

### Smoke test via HTTP (raw JSON-RPC)

```sh
./tempmail-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"tempmail_load","arguments":{"entity":"email","query":{"id":1}}}}'
```
