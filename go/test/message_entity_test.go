...
func messageBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"message01", "message02", "message03", "inbox01", "inbox02", "inbox03", "token01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("TEMPMAIL_TEST_MESSAGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TEMPMAIL_TEST_MESSAGE_ENTID"])
